...
if (is_post()) {
    ...
    try {
        ...
        if ($action === 'update_playlist') {
            ...
            music_customer_update_playlist($playlistId, (int)$user['id'], input('title'), input('description'));
        ...
        if ($action === 'add_track') {
            ...
            redirect($view === 'library'
                ...
                : 'portal/customer.php?view=playlists&id=' . $playlistId);
        }
        if ($action === 'remove_track') {
            $playlistId = int_input('playlist_id');
            music_customer_remove_track($playlistId, int_input('track_id'), (int)$user['id']);
        ...
        if ($action === 'change_password') {
            ...
            $statement = db()->prepare('SELECT password_hash FROM users WHERE id=:id AND role="customer"');
            ...
            if (!hash_equals($new, $confirm)) throw new RuntimeException('The new passwords do not match.');
            db()->prepare(
...
<main class="music-customer-main">
            ...
            <section class="music-customer-card">
                ...
                <div class="music-customer-card-body">
                    ...
                    <form method="post" class="music-customer-form" style="margin-top:18px">
                        ...
                        <label class="music-customer-field"><span>New playlist name</span><input name="title" maxlength="120" required></label>
                    ...
                    <?php else: ?>
                        ...
                        <?php if ($selectedPlaylist['tracks']): ?>
                            <table class="music-customer-table">
                                ...
                                <?php foreach ($selectedPlaylist['tracks'] as $track): ?>
                                    <tr>
                                        <td><div class="music-customer-track"><img src="<?=e(music_cover_url('track', (int)$track['id']))?>" alt=""><span><strong><?=e($track['title'])?></strong><span><?=e($track['artist_name'])?></span></span></div></td>
                                        ...
                                        <td><form method="post" class="music-customer-inline-form"><?=csrf_field()?><input type="hidden" name="action" value="remove_track"><input type="hidden" name="playlist_id" value="<?=(int)$selectedPlaylist['id']?>"><input type="hidden" name="track_id" value="<?=(int)$track['id']?>"><button type="submit">Remove</button></form></td>
                                    </tr>
                                <?php endforeach; ?>
                                </tbody>
                            </table>
                        ...
                        <div class="music-customer-account-grid" style="margin-top:18px">
                            ...
                            <form method="post" class="music-customer-form">
                                ...
                                <input type="hidden" name="playlist_id" value="<?=(int)$selectedPlaylist['id']?>">
                                <p style="margin:0;color:#6e7b89;font-size:12px;line-height:1.6">Deleting this playlist removes only its saved track list.</p>
                                <label class="music-customer-field"><span>Type DELETE to confirm</span><input name="delete_confirmation" pattern="DELETE" required></label>
    ...
    <?php if ($view === 'library'): ?>
        <section class="music-customer-card">
            ...
            <div class="music-customer-card-body">
                <?php if (!$playlists): ?><div class="music-customer-alert">Create a playlist before adding tracks. <a href="<?=e(app_url('portal/customer.php?view=playlists&new=1'))?>">Create playlist</a></div><?php endif; ?>
                <?php if ($tracks): ?>
                    <table class="music-customer-table">
                        ...
                        <?php foreach ($tracks as $track): ?>
                            <tr>
                                <td><div class="music-customer-track"><img src="<?=e($track['cover_url'])?>" alt=""><span><strong><?=e($track['title'])?></strong><span><?=e($track['artist'])?></span></span></div></td>
                                ...
                                <td>
                                    <?php if ($playlists): ?>
                                        <form method="post" class="music-customer-inline-form">
                                            ...
                                            <select name="playlist_id" aria-label="Playlist" required><?php foreach ($playlists as $playlist): ?><option value="<?=(int)$playlist['id']?>"><?=e($playlist['title'])?></option><?php endforeach; ?></select>
                ...
                <?php else: ?><div class="music-customer-empty">No published tracks are available.</div><?php endif; ?>
            </div>
        </section>
    <?php endif; ?>
