...
if (is_post()) {
    ...
    try {
        ...
        if ($action === 'validate_license') {
            ...
            flash('success', 'The VP3 entitlement was validated and managed updates were refreshed.');
...
if ($schemaAvailable) {
    try {
        ...
        $receipts = db()->query('SELECT * FROM vp3_license_validation_receipts ORDER BY created_at DESC,id DESC LIMIT 60')->fetchAll();
        $events = db()->query(
...
$policy = vp3_managed_updates_policy($current);
$updatesEnabled = (bool)$policy['automatic_updates_enabled'];
$licenseConfigured = (bool)$policy['license_configured'];
$statusClass = $updatesEnabled ? 'good' : ($licenseConfigured ? 'warn' : 'neutral');
...
<section class="vp3-panel"><span class="vp3-kicker">Current assignment</span><h2>License and deployment identity</h2><div class="vp3-stat-grid"><div class="vp3-stat"><span>Domain</span><strong><?=e((string)$current['domain']?:'Not provisioned')?></strong></div><div class="vp3-stat"><span>Plan</span><strong><?=e((string)$current['plan']?:'No managed-updates plan')?></strong></div><div class="vp3-stat"><span>License status</span><strong><?=$licenseConfigured?e(status_label((string)$current['status'])):'Not required for site use'?></strong></div><div class="vp3-stat"><span>Installed POD</span><strong><?=e((string)$current['installed_version'])?></strong></div></div><div class="vp3-grid" style="margin-top:18px"><div><p><strong>VP3 account</strong><br><span class="vp3-id"><?=e((string)$current['account_public_id']?:'Not provisioned')?></span></p><p><strong>Domain registration</strong><br><span class="vp3-id"><?=e((string)$current['domain_registration_id']?:'Not provisioned')?></span></p><p><strong>POD license</strong><br><span class="vp3-id"><?=e((string)$current['license_public_id']?:'Add later for managed updates')?></span></p></div><div><p><strong>POD deployment</strong><br><span class="vp3-id"><?=e((string)$current['deployment_id']?:'Not provisioned')?></span></p><p><strong>Installation fingerprint</strong><br><span class="vp3-id"><?=e((string)$current['installation_fingerprint'])?></span></p><p><strong>Provider</strong><br><?=e((string)$current['provider_name'])?> · <?=e($providerUrl)?></p></div></div></section>

<div class="vp3-grid">
<section class="vp3-panel"><span class="vp3-kicker">Managed-update validation</span><h2>Entitlement lease</h2><table class="vp3-table"><tbody><tr><th>Renewal</th><td><?=e(format_datetime((string)$current['renewal_at']))?></td></tr><tr><th>Entitlement expiration</th><td><?=e(format_datetime((string)$current['entitlement_expires_at']))?></td></tr><tr><th>Offline lease expiration</th><td><?=e(format_datetime((string)$current['offline_lease_expires_at']))?><?php if($current['offline_lease_valid']):?> <span class="vp3-badge good">Valid</span><?php endif;?></td></tr><tr><th>Last validation</th><td><?=e(format_datetime((string)$current['last_successful_validation_at']))?></td></tr><tr><th>Last heartbeat</th><td><?=e(format_datetime((string)$current['last_heartbeat_at']))?></td></tr><tr><th>Last error</th><td><?=e((string)$current['last_error_code']?:'None')?></td></tr></tbody></table><div class="vp3-actions" style="margin-top:16px"><form method="post"><?=csrf_field()?><input type="hidden" name="action" value="validate_license"><button class="vp3-button" type="submit" <?=$licenseConfigured?'':'disabled'?>>Validate license</button></form><form method="post"><?=csrf_field()?><input type="hidden" name="action" value="send_heartbeat"><button class="vp3-button secondary" type="submit" <?=$licenseConfigured?'':'disabled'?>>Send heartbeat</button></form><form method="post"><?=csrf_field()?><input type="hidden" name="action" value="refresh_jwks"><button class="vp3-button secondary" type="submit" <?=$licenseConfigured?'':'disabled'?>>Refresh public keys</button></form><form method="post" onsubmit="return confirm('Rotate the provisioned deployment credential now?');"><?=csrf_field()?><input type="hidden" name="action" value="rotate_credential"><button class="vp3-button danger" type="submit" <?=$licenseConfigured?'':'disabled'?>>Rotate credential</button></form></div></section>
<section class="vp3-panel"><span class="vp3-kicker">Storage</span><h2><?=e(format_bytes((int)$storage['used_bytes']))?> used</h2><p class="vp3-muted"><?php if($storage['allowance_bytes']!==null):?>of <?=e(format_bytes((int)$storage['allowance_bytes']))?> managed-plan storage<?php else:?>Local POD storage is not license-limited until a managed plan supplies an allowance.<?php endif;?> · <?=number_format((int)$storage['file_count'])?> files</p><div class="vp3-progress"><i style="width:<?=e((string)$storagePercent)?>%"></i></div><p><span class="vp3-badge <?=in_array($storage['warning_state'],['hard_limit','over_limit'],true)?'bad':(str_starts_with((string)$storage['warning_state'],'warning_')?'warn':'good')?>"><?=e(status_label((string)$storage['warning_state']))?></span></p><p class="vp3-muted">Existing files are never removed automatically. Unlicensed installations continue using their locally managed storage settings.</p><form method="post"><?=csrf_field()?><input type="hidden" name="action" value="measure_storage"><button class="vp3-button secondary" type="submit">Measure storage now</button></form></section>
</div>

<div class="vp3-grid"><section class="vp3-panel"><span class="vp3-kicker">Capabilities and limits</span><h2>Managed-service entitlement</h2><div class="vp3-entitlements"><?php if(!$entitlements):?><div class="vp3-empty">No managed-service entitlement is cached. Core POD features remain available.</div><?php else:?><?php foreach($entitlements as $key=>$value):?><div class="vp3-entitlement"><code><?=e((string)$key)?></code><strong><?=is_bool($value)?($value?'Allowed':'Not allowed'):e(is_scalar($value)?(string)$value:json_encode($value,JSON_UNESCAPED_SLASHES))?></strong></div><?php endforeach;?><?php endif;?></div></section><section class="vp3-panel"><span class="vp3-kicker">Policy</span><h2>License only the managed layer</h2><ul><li>The POD and its existing data remain usable without a license.</li><li>A verified VP3 entitlement is required for automated downloads and updates.</li><li>The owner can manually deploy code without licensing the automated updater.</li><li>A license can be added later without reinstalling or replacing the POD.</li><li>License expiration never deletes customer data.</li></ul></section></div>
