        ...
        <header class="portal-topbar">
            ...
            <div class="portal-header-user">
                <div class="portal-notification-wrap">
                    <button class="portal-notification-button" type="button" data-notification-toggle aria-expanded="false" aria-label="Open notifications">
                    ...
                    <section class="portal-notification-menu" data-notification-menu hidden>
                        <header><div><span>Activity feed</span><strong>Notifications</strong></div><a href="<?=e($notificationsUrl)?>">View all</a></header>
                        <div data-notification-preview-list>
                            <?php foreach ($recentNotifications as $notification): ?>
                                <a class="<?=(int)$notification['is_read'] === 0 ? 'unread' : ''?>" href="<?=e(notification_portal_link($user, $notification['link_url']))?>" data-notification-preview data-notification-id="<?=(int)$notification['id']?>">
                                    <span><?=e(notification_category_icon((string)$notification['category']))?></span>
                                    <span><strong><?=e($notification['title'])?></strong><small><?=e(format_datetime($notification['created_at']))?></small></span>
                                </a>
                            <?php endforeach; ?>
                            <?php if (!$recentNotifications): ?><div class="portal-notification-empty">No notifications yet.</div><?php endif; ?>
                        </div>
                    </section>
                </div>
    ...
    ?>
        ...
        <?php if ($isAdmin): ?>
            ...
            <section class="admin-assistant-chat" data-admin-assistant-chat aria-label="North Mountain administrator assistant" hidden>
                <header><div><span>North Mountain Admin Assistant</span><strong>Operations chat</strong></div><div><button type="button" data-admin-chat-new>New chat</button><button type="button" data-admin-chat-close aria-label="Close assistant">×</button></div></header>
            ...
            <section class="admin-assistant-footer" data-admin-assistant-footer>
                ...
                <div class="admin-assistant-quick-menu admin-assistant-launcher-modal" data-admin-assistant-quick-menu role="dialog" aria-modal="true" aria-labelledby="admin-launcher-title" hidden>
                    ...
                    <nav class="admin-assistant-launcher-tabs" role="tablist" aria-label="Administrator tool categories">
                        <button type="button" id="admin-launcher-tab-queries" role="tab" aria-selected="true" aria-controls="admin-launcher-panel-queries" data-admin-launcher-tab="queries" class="is-active">Data queries</button>
                        <button type="button" id="admin-launcher-tab-publishing" role="tab" aria-selected="false" aria-controls="admin-launcher-panel-publishing" data-admin-launcher-tab="publishing">Publishing</button>
                        <button type="button" id="admin-launcher-tab-actions" role="tab" aria-selected="false" aria-controls="admin-launcher-panel-actions" data-admin-launcher-tab="actions">Actions</button>
                    </nav>
                    <div class="admin-assistant-launcher-body">
                        <section class="admin-assistant-launcher-panel is-active" id="admin-launcher-panel-queries" role="tabpanel" aria-labelledby="admin-launcher-tab-queries" data-admin-launcher-panel="queries">
                            <div class="admin-assistant-launcher-intro"><strong>Quick data queries</strong><span>Ask the assistant to review live portal records.</span></div>
                            <div class="admin-assistant-query-grid">
                                ...
                                <?php if (nmm_module_enabled('music_library')): ?><button type="button" data-admin-quick-prompt="Music Library"><strong>Music Library</strong><small>Review catalog and listening activity</small></button><?php endif; ?>
                                ...
                                <?php if (nmm_module_enabled('clients')): ?><button type="button" data-admin-quick-prompt="Unread communications"><strong>Communications</strong><small>Show unread conversations</small></button><?php endif; ?>
                                <?php if (nmm_module_enabled('clients')): ?><button type="button" data-admin-quick-prompt="Open projects"><strong>Open projects</strong><small>Summarize current project work</small></button><?php endif; ?>
                            </div>
                        </section>

                        <section class="admin-assistant-launcher-panel" id="admin-launcher-panel-publishing" role="tabpanel" aria-labelledby="admin-launcher-tab-publishing" data-admin-launcher-panel="publishing" hidden>
                            <div class="admin-assistant-launcher-intro"><strong>Create and publish</strong><span>Only post types enabled in Settings appear here. Forms open inside this workspace, and every choice remains a valid direct link.</span></div>
                            <div class="footer-publishing-layout" data-footer-publishing>
                                ...
                                <section class="footer-publishing-stage" aria-live="polite">
                                    ...
                                    <div class="footer-publishing-loading" data-footer-publishing-loading hidden>Loading publishing form…</div>
                        ...
                        <section class="admin-assistant-launcher-panel" id="admin-launcher-panel-actions" role="tabpanel" aria-labelledby="admin-launcher-tab-actions" data-admin-launcher-panel="actions" hidden>
                            <div class="admin-assistant-launcher-intro"><strong>Dashboard actions</strong><span>Open operational tools and settings.</span></div>
                            <div class="admin-assistant-action-grid">
                                <?php if (nmm_module_enabled('call_us')): ?><a href="<?=e(app_url('portal/admin.php?view=call-center'))?>"><span>Open</span><strong>Call Center</strong><small>Calls, voicemail, and callbacks</small></a><?php endif; ?>
                                <?php if (nmm_module_enabled('clients')): ?><a href="<?=e(app_url('portal/admin.php?view=communications'))?>"><span>Open</span><strong>Communications</strong><small>Messages and conversations</small></a><?php endif; ?>
                                ...
                                <?php if (nmm_module_enabled('landing_page')): ?><a href="<?=e(app_url('portal/admin.php?view=builder'))?>"><span>Build</span><strong>Page Editor</strong><small>Pages, sections, and blocks</small></a><?php endif; ?>
                ...
                <form class="admin-assistant-composer" data-admin-assistant-form>
                    <button class="admin-assistant-plus" type="button" data-admin-quick-toggle aria-expanded="false" aria-label="Open administrator tools">+</button>
                    <textarea rows="1" maxlength="500" data-admin-assistant-input placeholder="Ask about calls, messages, CRM contacts, projects, clients, or notifications…" aria-label="Ask the administrator assistant"></textarea>
...
<section class="portal-confirm-modal" data-confirm-modal aria-hidden="true" hidden>
    ...
    <div class="portal-confirm-dialog" role="dialog" aria-modal="true" aria-labelledby="portal-confirm-title" aria-describedby="portal-confirm-message">
        <div class="portal-confirm-icon" aria-hidden="true">!</div>
        <div class="portal-confirm-copy"><span data-confirm-eyebrow>Confirm action</span><h2 id="portal-confirm-title" data-confirm-title>Are you sure?</h2><p id="portal-confirm-message" data-confirm-message>This action cannot be undone.</p></div>
        <div class="portal-confirm-actions"><button class="button" type="button" data-confirm-cancel>Cancel</button><button class="button button-danger" type="button" data-confirm-accept>Continue</button></div>
    </div>
</section>
