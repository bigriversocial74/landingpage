...
function password_policy_errors(string $password, string $email = ''): array
{
    ...
    if ($emailLocal !== '' && strlen($emailLocal) >= 4 && str_contains(strtolower($password), $emailLocal)) $errors[] = 'Password must not contain the email username.';
    return $errors;
}
...
function rate_limit_exceeded(string $action, string $identity, int $limit, int $windowSeconds): bool
{
    ...
    try {
        ...
        $events = array_values(array_filter($_SESSION[$key] ?? [], static fn($timestamp): bool => (int)$timestamp >= time() - $windowSeconds));
        $events[] = time();
        $_SESSION[$key] = $events;
...
function profile_columns_available(): bool
{
    ...
    try {
        $statement = db()->query('SELECT COUNT(*) FROM information_schema.columns WHERE table_schema=DATABASE() AND table_name="users" AND column_name IN ("profile_image_stored_name","profile_image_mime","profile_image_updated_at")');
...
function profile_image_storage_directory(): string
{
    ...
    if (!is_dir($directory) && !mkdir($directory, 0750, true) && !is_dir($directory)) throw new RuntimeException('The profile-image storage directory could not be created.');
    return $directory;
}
...
function user_profile_image_url(?array $user): string
{
    if (!$user || empty($user['id']) || empty($user['profile_image_stored_name'])) {
        return app_url(($user['role'] ?? 'admin') === 'admin' ? 'assets/images/david-evans-profile.jpg' : 'assets/images/profile-placeholder.svg');
    }
...
function primary_admin_profile(): ?array
{
    ...
    try {
        $profileFields = profile_columns_available() ? 'profile_image_stored_name,profile_image_mime,profile_image_updated_at' : 'NULL AS profile_image_stored_name,NULL AS profile_image_mime,NULL AS profile_image_updated_at';
        $statement = db()->query('SELECT id,role,email,display_name,company,phone,status,' . $profileFields . ',updated_at FROM users WHERE role="admin" AND status="active" ORDER BY id ASC LIMIT 1');
...
{
    if (!profile_columns_available()) throw new RuntimeException('Import database/account_profile_v38.sql before saving account profile settings.');
    ...
    if ($removeImage && !$newImageUploaded) { $newStoredName = ''; $newMime = null; }
    $pdo = db();
    try {
        $pdo->beginTransaction();
        $statement = $pdo->prepare('UPDATE users SET display_name=:display_name,email=:email,company=:company,phone=:phone,profile_image_stored_name=:profile_image_stored_name,profile_image_mime=:profile_image_mime,profile_image_updated_at=CASE WHEN :profile_changed=1 THEN UTC_TIMESTAMP() ELSE profile_image_updated_at END WHERE id=:user_id');
        $statement->execute(['display_name' => $displayName, 'email' => $email, 'company' => $company !== '' ? $company : null, 'phone' => $phone !== '' ? $phone : null, 'profile_image_stored_name' => $newStoredName !== '' ? $newStoredName : null, 'profile_image_mime' => $newStoredName !== '' ? $newMime : null, 'profile_changed' => ($newImageUploaded || $removeImage) ? 1 : 0, 'user_id' => $userId]);
        $pdo->commit();
    ...
    if (($newImageUploaded || $removeImage) && $currentStoredName !== '' && $currentStoredName !== $newStoredName) @unlink(profile_image_storage_directory() . '/' . basename($currentStoredName));
...
function current_user(): ?array
{
    ...
    $profileFields = profile_columns_available() ? 'profile_image_stored_name,profile_image_mime,profile_image_updated_at' : 'NULL AS profile_image_stored_name,NULL AS profile_image_mime,NULL AS profile_image_updated_at';
    $statement = db()->prepare('SELECT id,role,email,display_name,company,phone,status,must_change_password,' . $profileFields . ',last_login_at,created_at FROM users WHERE id=:id LIMIT 1');
    $statement->execute(['id' => $id]);
    $user = $statement->fetch();
    if (!$user || $user['status'] !== 'active') { unset($_SESSION['user_id']); return null; }
    $cachedId = $id;
    $cached = $user;
    return $user;
}
...
{
    try {
        $statement = db()->prepare('INSERT INTO activity_log (user_id,event_type,entity_type,entity_id,metadata_json) VALUES (:user_id,:event_type,:entity_type,:entity_id,:metadata_json)');
        $statement->execute(['user_id' => current_user()['id'] ?? null, 'event_type' => $event, 'entity_type' => $entityType, 'entity_id' => $entityId, 'metadata_json' => $metadata ? json_encode($metadata, JSON_THROW_ON_ERROR) : null]);
    } catch (Throwable) {}
}
...
function attempt_login(string $email, string $password, string $role): bool
{
    ...
    db()->prepare('INSERT INTO login_attempts (email,ip_address,success) VALUES (:email,:ip_address,:success)')->execute(['email' => $email, 'ip_address' => $ip, 'success' => $valid ? 1 : 0]);
...
function logout_user(): void
{
    ...
    if (ini_get('session.use_cookies')) {
        ...
        setcookie(session_name(), '', ['expires' => time() - 42000, 'path' => $parameters['path'], 'domain' => $parameters['domain'], 'secure' => $parameters['secure'], 'httponly' => $parameters['httponly'], 'samesite' => $parameters['samesite'] ?? 'Lax']);
    }
    session_destroy();
}
...
{
    $user = current_user();
    if (!$user || $user['role'] !== $role) redirect('portal/login.php?role=' . urlencode($role));
    if ((int)$user['must_change_password'] === 1 && (string)($_GET['view'] ?? '') !== 'account') redirect('portal/' . ($role === 'admin' ? 'admin.php' : 'client.php') . '?view=account&required=1');
    return $user;
}
...
function enforce_authenticated_action_limit(array $user): void
{
    ...
    $limit = $isAdmin ? max(10, (int)($security['admin_action_limit'] ?? 120)) : max(5, (int)($security['client_action_limit'] ?? 45));
    $window = max(30, (int)($security['action_window_seconds'] ?? 60));
    if (rate_limit_exceeded($isAdmin ? 'admin_action' : 'client_action', (string)$user['id'], $limit, $window)) { http_response_code(429); exit('Too many actions were submitted. Wait briefly and try again.'); }
}
...
<body class="portal-body" data-portal-role="<?=e((string)$user['role'])?>" data-notification-api="<?=e($notificationApiUrl)?>" data-call-center-api="<?=e($callCenterApiUrl)?>" data-admin-assistant-api="<?=e($adminAssistantApiUrl)?>" data-portal-active="<?=e($active)?>">
<div class="portal-shell">
    <?php require __DIR__ . '/sidebar.php'; ?>
    <button class="portal-sidebar-backdrop" data-sidebar-close type="button" aria-label="Close navigation"></button>
    <main class="portal-main">
        <header class="portal-topbar">
            <button class="portal-menu-button" data-sidebar-open type="button" aria-label="Open navigation"><span></span><span></span><span></span></button>
            <?php nmm_render_mobile_brand(); ?>
            <div class="portal-title-block"><span><?=e($isAdmin ? 'North Mountain Media' : 'Client workspace')?></span><h1><?=e($title)?></h1></div>
            <div class="portal-header-user">
                <div class="portal-notification-wrap">
                    <button class="portal-notification-button" type="button" data-notification-toggle aria-expanded="false" aria-label="Open notifications"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M18 8a6 6 0 0 0-12 0c0 7-3 7-3 9h18c0-2-3-2-3-9"></path><path d="M10 21h4"></path></svg><span data-notification-count <?=$notificationCount > 0 ? '' : 'hidden'?>><?=$notificationCount?></span></button>
                    <section class="portal-notification-menu" data-notification-menu hidden><header><div><span>Activity feed</span><strong>Notifications</strong></div><a href="<?=e($notificationsUrl)?>">View all</a></header><div data-notification-preview-list><?php foreach ($recentNotifications as $notification): ?><a class="<?=(int)$notification['is_read'] === 0 ? 'unread' : ''?>" href="<?=e(notification_portal_link($user, $notification['link_url']))?>" data-notification-preview data-notification-id="<?=(int)$notification['id']?>"><span><?=e(notification_category_icon((string)$notification['category']))?></span><span><strong><?=e($notification['title'])?></strong><small><?=e(format_datetime($notification['created_at']))?></small></span></a><?php endforeach; ?><?php if (!$recentNotifications): ?><div class="portal-notification-empty">No notifications yet.</div><?php endif; ?></div></section>
                </div>
...
function portal_footer(): void
{
    ...
    ?>
        ...
        <?php if ($isAdmin): ?>
            <section class="admin-assistant-loading" data-admin-assistant-loading aria-live="polite" hidden><div class="admin-assistant-loading-orb" aria-hidden="true"><span></span><span></span><span></span></div><strong>Querying North Mountain data</strong><small>Reviewing calls, communications, CRM, and current work.</small></section>
            <section class="admin-assistant-chat" data-admin-assistant-chat aria-label="North Mountain administrator assistant" hidden><header><div><span>North Mountain Admin Assistant</span><strong>Operations chat</strong></div><div><button type="button" data-admin-chat-new>New chat</button><button type="button" data-admin-chat-close aria-label="Close assistant">×</button></div></header><div class="admin-assistant-messages" data-admin-assistant-messages aria-live="polite"></div></section>
            <section class="admin-assistant-footer" data-admin-assistant-footer>
                <button class="admin-assistant-launcher-backdrop" type="button" data-admin-launcher-backdrop aria-label="Close administrator tools" hidden></button>
                <div class="admin-assistant-quick-menu admin-assistant-launcher-modal" data-admin-assistant-quick-menu role="dialog" aria-modal="true" aria-labelledby="admin-launcher-title" hidden>
                    <header class="admin-assistant-launcher-header"><div><span>Administrator tools</span><strong id="admin-launcher-title">Quick access</strong></div><button type="button" data-admin-quick-close aria-label="Close administrator tools">×</button></header>
                    <nav class="admin-assistant-launcher-tabs" role="tablist" aria-label="Administrator tool categories"><button type="button" id="admin-launcher-tab-queries" role="tab" aria-selected="true" aria-controls="admin-launcher-panel-queries" data-admin-launcher-tab="queries" class="is-active">Data queries</button><button type="button" id="admin-launcher-tab-publishing" role="tab" aria-selected="false" aria-controls="admin-launcher-panel-publishing" data-admin-launcher-tab="publishing">Publishing</button><button type="button" id="admin-launcher-tab-actions" role="tab" aria-selected="false" aria-controls="admin-launcher-panel-actions" data-admin-launcher-tab="actions">Actions</button></nav>
                    <div class="admin-assistant-launcher-body">
                        <section class="admin-assistant-launcher-panel is-active" id="admin-launcher-panel-queries" role="tabpanel" aria-labelledby="admin-launcher-tab-queries" data-admin-launcher-panel="queries"><div class="admin-assistant-launcher-intro"><strong>Quick data queries</strong><span>Ask the assistant to review live portal records.</span></div><div class="admin-assistant-query-grid"><button type="button" data-admin-quick-prompt="Most recent call history"><strong>Recent calls</strong><small>Review the latest call history</small></button><button type="button" data-admin-quick-prompt="Missed messages"><strong>Missed messages</strong><small>Find calls and messages needing attention</small></button><button type="button" data-admin-quick-prompt="CRM contacts needing attention"><strong>CRM attention</strong><small>Surface contacts needing follow-up</small></button><?php if (nmm_module_enabled('music_library')): ?><button type="button" data-admin-quick-prompt="Music Library"><strong>Music Library</strong><small>Review catalog and listening activity</small></button><?php endif; ?><button type="button" data-admin-quick-prompt="Visitor activity"><strong>Visitor activity</strong><small>Inspect recent website engagement</small></button><button type="button" data-admin-quick-prompt="Unread communications"><strong>Communications</strong><small>Show unread conversations</small></button><button type="button" data-admin-quick-prompt="Open projects"><strong>Open projects</strong><small>Summarize current project work</small></button></div></section>
                        <section class="admin-assistant-launcher-panel" id="admin-launcher-panel-publishing" role="tabpanel" aria-labelledby="admin-launcher-tab-publishing" data-admin-launcher-panel="publishing" hidden><div class="admin-assistant-launcher-intro"><strong>Create and publish</strong><span>Only post types enabled in Settings appear here. Every option opens its full page directly.</span></div><?php publishing_center_render_footer_links(); ?></section>
                        <section class="admin-assistant-launcher-panel" id="admin-launcher-panel-actions" role="tabpanel" aria-labelledby="admin-launcher-tab-actions" data-admin-launcher-panel="actions" hidden><div class="admin-assistant-launcher-intro"><strong>Dashboard actions</strong><span>Open operational tools and settings.</span></div><div class="admin-assistant-action-grid"><?php if (nmm_module_enabled('call_us')): ?><a href="<?=e(app_url('portal/admin.php?view=call-center'))?>"><span>Open</span><strong>Call Center</strong><small>Calls, voicemail, and callbacks</small></a><?php endif; ?><?php if (nmm_module_enabled('clients')): ?><a href="<?=e(app_url('portal/admin.php?view=communications'))?>"><span>Open</span><strong>Communications</strong><small>Messages and conversations</small></a><?php endif; ?><a href="<?=e(app_url('portal/admin.php?view=crm'))?>"><span>Open</span><strong>CRM</strong><small>Contacts and opportunities</small></a><a href="<?=e(app_url('portal/admin.php?view=analytics'))?>"><span>Open</span><strong>Visitor Intelligence</strong><small>Traffic and conversion analytics</small></a><a href="<?=e(app_url('portal/admin.php?view=site-analytics'))?>"><span>Open</span><strong>Site Analytics</strong><small>Website and media activity</small></a><?php if (nmm_module_enabled('landing_page')): ?><a href="<?=e(app_url('portal/admin.php?view=builder'))?>"><span>Build</span><strong>Page Editor</strong><small>Pages, sections, and blocks</small></a><?php endif; ?><a href="<?=e(app_url('portal/admin.php?view=menus'))?>"><span>Manage</span><strong>Navigation</strong><small>Menus and locations</small></a><a href="<?=e(app_url('portal/admin.php?view=settings'))?>"><span>Manage</span><strong>Settings</strong><small>Modules and publishing availability</small></a></div></section>
                    </div>
                </div>
                <form class="admin-assistant-composer" data-admin-assistant-form><button class="admin-assistant-plus" type="button" data-admin-quick-toggle aria-expanded="false" aria-label="Open administrator tools">+</button><textarea rows="1" maxlength="500" data-admin-assistant-input placeholder="Ask about calls, messages, CRM contacts, projects, clients, or notifications…" aria-label="Ask the administrator assistant"></textarea><button class="admin-assistant-submit" type="submit" aria-label="Send query">↑</button></form><small>Uses protected, predefined queries against the live portal database.</small>
            </section>
        <?php endif; ?>
    </main>
</div>
<section class="portal-confirm-modal" data-confirm-modal aria-hidden="true" hidden><button class="portal-confirm-backdrop" type="button" data-confirm-cancel aria-label="Cancel confirmation"></button><div class="portal-confirm-dialog" role="dialog" aria-modal="true" aria-labelledby="portal-confirm-title" aria-describedby="portal-confirm-message"><div class="portal-confirm-icon" aria-hidden="true">!</div><div class="portal-confirm-copy"><span data-confirm-eyebrow>Confirm action</span><h2 id="portal-confirm-title" data-confirm-title>Are you sure?</h2><p id="portal-confirm-message" data-confirm-message>This action cannot be undone.</p></div><div class="portal-confirm-actions"><button class="button" type="button" data-confirm-cancel>Cancel</button><button class="button button-danger" type="button" data-confirm-accept>Continue</button></div></div></section>
