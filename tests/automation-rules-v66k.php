...
$adminController = file_get_contents($root . '/portal/admin.php');
...
foreach ([$core, $admin, $adminController, $notifications, $recovery, $worker, $javascript, $css, $migration] as $source) {
...
v66k_assert(str_contains($adminController, "require_once __DIR__ . '/unified-inbox.php'"), 'The retained administrator controller must preserve Unified Inbox integration.');
v66k_assert(str_contains($adminController, "'settings','account'"), 'The retained administrator controller must preserve the System Settings route contract.');
v66k_assert(str_contains($notifications, "basename((string)(\$_SERVER['SCRIPT_FILENAME'] ?? ''))"), 'Automation routing must be scoped to the administrator front controller.');
v66k_assert(str_contains($notifications, "(string)(\$_GET['view'] ?? '') !== 'automation'"), 'The administrator integration must preserve every non-automation route.');
v66k_assert(str_contains($notifications, "require_once __DIR__ . '/automation-admin.php'"), 'The administrator integration must load the Action Center module.');
v66k_assert(str_contains($notifications, 'automation_handle_admin_action'), 'The Action Center POST handler must be connected.');
v66k_assert(str_contains($notifications, 'automation_render_admin'), 'The Action Center renderer must be connected.');
v66k_assert(str_contains($notifications, 'data-admin-navigation'), 'The permanent administrator navigation must expose the Action Center.');
