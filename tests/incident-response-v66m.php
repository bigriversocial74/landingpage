...
foreach (compact('core','admin','worker','migration','fresh','spec','style') as $name => $source) {
...
recovery_source_assert(strpos($core, 'SELECT * FROM recovery_executions WHERE idempotency_key=:idempotency_key') < strpos($core, 'The recovery simulation is stale or expired.'), 'Execution idempotency is evaluated too late.');
