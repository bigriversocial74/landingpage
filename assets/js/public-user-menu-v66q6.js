...
(() => {
  ...
  const runtimeScript = document.currentScript
    || document.querySelector('script[src*="public-user-menu-v66q6.js"]');
  ...
  const installStyles = () => {
    ...
    style.textContent = `
      ...
      .public-user-menu-panel{position:absolute;top:calc(100% + 10px);right:0;display:grid;min-width:220px;padding:8px;border:1px solid #dfe6ec;border-radius:15px;background:#fff;box-shadow:0 20px 50px rgba(18,32,48,.18)}
  ...
  const buildMenu = (host, existingLogin = null) => {
    ...
    panel.append(heading, client, admin);
  ...
  const initialize = () => {
    ...
    if (landingNav) buildMenu(landingNav, landingNav.querySelector('.landing-login'));
